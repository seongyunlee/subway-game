...
saveButton.addEventListener("click", captureGuideAndEnroll);
...
function enrollProblem(stationName, dataURI) {
    const url = `/admin/fillblank/problemList/save`;
    ...
    formData.append("problemImage", blob, `${stationName}.png`);
...
function captureGuideAndEnroll() {
    ...
    html2canvas(map).then((canvas) => {
        ...
        img.onload = () => {
            ...
            const uri = cropCanvas.toDataURL('image/png');

            enrollProblem(maskText.value.trim(), uri);
            a.url = uri;
